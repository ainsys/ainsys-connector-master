...
$settings_nav_tabs= $admin_ui->get_nav_fields();
...
<div class="wrap ainsys_settings_wrap">
	...
	<div class="ainsys-tabs-nav-wrapper">
		...
		<?php foreach ( $settings_nav_tabs as $nav_tab_key => $nav_tab ): ?>
			<a class="nav-tab <?php echo esc_attr( $nav_tab['active'] ) ? 'nav-tab-active' : ''; ?>"
			   href="#setting_section_<?php echo esc_attr( $nav_tab_key ) ?>"
			   data-target="setting_section_<?php echo esc_attr( $nav_tab_key ) ?>"><?php echo esc_html( $nav_tab['label'] ) ?></a>
		<?php endforeach; ?>
	...
	<div class="ainsys-tabs-content-wrapper">
			...
			<div class="ainsys-settings-blocks">
				<div class="ainsys-settings-block ainsys-settings-block--connection">
					...
					<form method="post" action="options.php">
						...
						for ( $i = 1; $i < 10; $i++ ) {
							?>
							<div class="ainsys-form-group ainsys-email<?php if ( ! empty( $admin_ui->settings::get_backup_email( $i ) ) ) { echo ' ainsys-email-show'; } //phpcs:ignore ?>" data-block-id="<?=$i?>">
								<label for="backup-email-<?php echo $i; ?>" class="ainsys-form-label">
									<?php _e( 'E-mail for error reports', AINSYS_CONNECTOR_TEXTDOMAIN ); // phpcs:ignore ?>
									<span class="ainsys-form-label-note"><?php _e( 'Additional email error reports', AINSYS_CONNECTOR_TEXTDOMAIN ); // phpcs:ignore ?></span>
								</label>
								<div class="ainsys-form-input">
						...
						<div class="ainsys-form-group ainsys-form-group-checkbox">
							...
							<label for="full-uninstall-checkbox" class="ainsys-form-label">
								...
								<span class="ainsys-form-label-note"><?php _e( 'NB: if you delete the plugin from WordPress admin panel it will clear data regardless of this checkbox', AINSYS_CONNECTOR_TEXTDOMAIN ); // phpcs:ignore ?></span>
							</label>
						</div>
						<div class="ainsys-form-group">
		...
		<div id="setting_section_entities" class="tab-target">
